...
all_users.uniq.each do |user| 
  ...
  user user[:id] do
	...
	# Create all needed groups, and add user to it
  user[:groups].each do |g|
	...
	# Give sudo rights to the users that need it
	if user["should_have_sudo"] then
		script "make_user_sudo" do
			interpreter "bash"
			code "sudo adduser #{user[:id]} sudo"
		end
	end
